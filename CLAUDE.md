# study-hub — 郵政升等考讀書 PWA

純靜態 PWA（無建置步驟），手機透過 GitHub Pages 使用。所有作答資料存在裝置 localStorage，靠 app 內的「匯出」按鈕（複製 JSON 到剪貼簿）貼給 Claude 回流到 repo。

> 資料流向、localStorage key 一覽、哪些檔案 app 會讀回 → 見 [README.md](README.md)。動到 `app.js` 的匯出邏輯或 `data/` 結構時，順手確認 README 是否要同步。

**規則分工**：本檔服務「手機雲端 session」——只做匯出內容的合併、commit、push。完整讀書流程（Notion 錯題回收、單字音檔生成、作文素材落地）在使用者 Mac 本機的考試 rules 裡，雲端做不了的（如音檔）在回覆結尾提醒使用者回 Mac 補即可。

## 收到匯出內容時的處理規則

使用者會貼上以【…匯出】開頭的 JSON，依開頭字樣處理：

### 【讀書紀錄匯出，請併入 data/records.json】

這份是**每日主要匯出**，同時涵蓋紀錄存檔與錯題回收，正常情況下一天只會收到這一份。

1. 讀取 `data/records.json`（以日期為 key 的物件）。
2. 以匯出內容的 `date` 為 key 寫入整包資料（去掉外層 `type` 與 `pendingWrong` 欄位——`pendingWrong` 只供排回收，不入檔）。
3. 同日期已有資料時直接覆蓋——匯出是當日累計快照，晚匯出的一定比較完整。
4. **接著排隔日回收**：把 `drill.wrong` ＋ `pendingWrong`（若有）依「【刷題錯題匯出】」的規則寫入 `data/progress.json` 隔日的 `items`。
5. 完成後 commit，訊息格式：`records: 併入 YYYY-MM-DD 讀書紀錄`；若同時動到 progress.json，分兩個 commit。

欄位說明：

- `drill.count`＝今日刷題數；`drill.wrong`＝今日新錯題；**`drill.right`＝今日答對的題號**（`{id, subject, point}`，同日同題只記一次）
  - `drill.right` 於 2026-08-14 新增。在此之前答對只是從錯題本移除、不留紀錄，匯出帶不出去，Mac 端就無法把 Notion 對應頁面自動標為已回收（8/12、8/14 各發生一次，91 題只能人工對帳）。回收卷每題都綁 pageId，有 id 即可自動銷帳。舊資料沒有此欄屬正常。
- `pendingWrong`＝前幾天漏匯出的錯題（僅在有殘留時出現，一併排回收）
- `exams`＝今日模考成績
- `vocab.done` / `vocab.right`＝今日單字實際作答數與答對數（流水計數）
- `vocabStages`＝單字熟練度分佈快照（0~3 級的字數，僅含已接觸過的字）

`vocab.done` 與 `vocabStages` 要一起看：stage 分佈是快照，字滿 3 級後就不再變動，光看分佈會低估當日用功程度；`done` 才是當天實際翻了幾個。舊資料（2026-07-21 以前）沒有 `vocab` 欄位屬正常。

### 【自訂單字匯出，請補完詞性與例句後併入 vocab.json】

1. **Mac 端（有本機環境）**：逐字跑 `scripts/add_word.py <word> --pos … --zh … --ex …`——會一併生成發音音檔，不要手動編輯 vocab.json。
2. **雲端 session（無法跑 macOS `say`）**：為每個字補上 `pos`（詞性縮寫如 `n.` `v.` `adj.`）與 `ex`（郵政／考試情境的英文例句尤佳），併入 `data/vocab.json`（`w` 已存在者跳過；保留 `custom: true`），並在回覆結尾提醒：「回 Mac 後補跑音檔生成」。
3. commit 訊息格式：`vocab: 新增自訂單字 <字1>、<字2>`。

### 【作文短段匯出，請批改】

短篇作文（約 100～150 字）批改。依序給：

1. **總評**（1～2 句：扣題程度、結構、語氣）。
2. **逐項修改建議**：具體指出哪句怎麼改、為什麼；錯字與標點一併挑出。
3. **示範改寫**：給一段修改後的完整版本，保留使用者原本的論點與素材，不要整段重寫成另一篇；也請保留使用者選用的題材——即使是備考文章，內容不必是郵政情境，未經要求請勿把舉例或語境改寫成郵政／公務框架。
4. 若為考試作文短段練習，提醒可再延伸的論點方向 1～2 個。

批改結果直接回覆即可，不需寫入 repo 檔案。

### 【刷題錯題匯出，請依複習流程處理】

**備用管道**：日常錯題回收已併入「讀書紀錄匯出」，這份只在使用者按「單獨匯出錯題」或「重匯全部」時才會出現（例如想單獨補排、或重排已匯出過的題）。

錯題回收主要在 Mac 端搭配 Notion 處理。若在雲端 session 收到，將錯題整理成隔日回收題項目，寫入 `data/progress.json` 對應日期的 `items`（每題一行、附考點），並告知使用者已排入。

寫項目時每題一行，格式為 `科目｜考點（題號）：關鍵觀念，附一句怎麼複習`；同一節或高度相關的題可在行末互相指涉。若當日某科連錯 3 題以上，額外加一行提醒排該科小複習。

若收到的錯題已排過（同題號已在隔日 `items` 裡），直接略過並告知，不重複寫入。

**`options` 是當輪洗牌後的選項原文（2026-08-15 新增）**，與同一筆的 `user`／`answer`／`userText` 屬同一個字母空間，**四者必須整組一起用**。用途是題庫查不到該題 id 時仍能還原完整題面——在此之前匯出不帶選項，Mac 端只寫得出題幹，那筆從此無源（稽核當日英文庫到期 71 題有 52 題如此，其中 39 題連本機三條救援路徑都還原不回來）。舊資料沒有此欄屬正常。

**`user` 與 `answer` 是當輪洗牌後的字母，不能拿去對題庫的選項順序。**app 每輪重洗選項防止背答案位置，所以匯出的字母只在那一輪有效；要指出她選了哪個選項，看同時附上的 `userText`（選項原文）。忽略這點會標出指向別的選項的「正解」——2026-08-10 稽核發現 Notion 訂正庫 39 筆可核對條目中有 8 筆因此標錯。

## 通用規範

- 回應一律使用繁體中文。
- JSON 檔以 `indent=1`、UTF-8（不轉義中文）寫入，維持既有格式。
- 修改後 commit；在雲端 session 中同時 push（手機 PWA 靠 GitHub Pages 重新部署後才看得到更新）。
