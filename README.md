...
```jsonc
{
  ...
  "exams": [ … ],       // 今日試卷成績；含 id 與逐題 wrongs（2026-08-17 起）
  "vocab": {            // 今日單字流水計數
    "done": 15,         // 實際作答字數
    "right": 11
  },
...
**試卷（模考／迷你卷）的錯題走同一份匯出。**`exams[].wrongs` 帶題號、考點、你選與正解（2026-08-17 起）。在此之前只帶分數欄，迷你卷刷完 Mac 端只看得到「9/12」，排不出回收也對不回是哪個考點沒過——成績頁那顆「匯出作答紀錄」才拿得到逐題，等於多一道手動步驟。
